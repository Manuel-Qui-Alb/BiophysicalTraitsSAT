...
def myTSEB_binomial(LAI, fv_var, h_V, row_sep, x_LAD, phi_degrees,
    ...
    Sn_V, Sn_S = compute_binomial_prism_manuel(
        ...
        sza=sza_rad,  # sun zenith angle (radians)
        psi=phi_rad,  # sun azimuth relative to row orientation (radians)
        ...
        Srad_dir=Sdn_dir,  # Direct-beam incoming radiation (W m-2)
        Srad_diff=Sdn_dif,  # Diffuse incoming radiation (W m-2)
    ...
    while (np.any(loop_con) and iterations <= max_iterations) :
        ...
        if const_L is None:
            ...
            u_friction[loop_con] = np.asarray(np.maximum(U_FRICTION_MIN[loop_con], u_friction[loop_con]),
                                              dtype=np.float32)
...
print(f"Execution time BINOMIAL: {elapsed_time} seconds")
...
df_inputs.loc[:, 'Trad_V_BIN'] = Trad_V
df_inputs.loc[:, 'Trad_S_BIN'] = Trad_S
df_inputs.loc[:, 'Ln_V_BIN'] = Ln_V
df_inputs.loc[:, 'Ln_S_BIN'] = Ln_S
df_inputs.loc[:, 'Rn_V_BIN'] = Rn_V
df_inputs.loc[:, 'Rn_S_BIN'] = Rn_S
df_inputs.loc[:, 'LE_V_BIN'] = LE_V
df_inputs.loc[:, 'LE_S_BIN'] = LE_S

df_inputs.to_csv('files/TSEB_BINOMIAL.csv')
...
# Rn_V, Rn_S
path = 'files/sensitivity_analysis_TSEB_BINOMIAL.xlsx'
