...
def binomial_model(lai, fv_var, h_V, row_sep,
                   # plant_sep_var,
...
problem = {
    ...
    "names": names,
    "bounds": [
        [0.5, 2.5],  # lai: orchards/crops
        [0.7, 0.85],  # fv_var
        [1, 1.5],  # CanopyHeight
        [3.0, 4.0],  # row_sep: typical orchard rows
        # [0.5, 1],  # plant_sep_var: plant space in relation to sr
        [100, 300],  # doy: avoid near 0 and near 90
        [11, 16], # hour: relative azimuth 0..pi
        [np.deg2rad(0), np.deg2rad(180)], # row_orientation: relative azimuth 0..pi
        [0.80, 0.85],  # 18. abs_vis_leaf
        [0.35, 0.42],  # 19. abs_nir_leaf
