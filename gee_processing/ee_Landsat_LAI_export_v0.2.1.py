...
def getLandsat(start, end, path, row):
    ...
    landsat_sr_coll = landsat_sr_coll \
        ...
        .filterBounds(vector)#.filter(ee.Filter.lt('CLOUD_COVER',70)) \
...
#   laiBand = image.select('LAI').multiply(0.01)
#   thermalBands = image.select('ST_B.*').multiply(0.00341802).add(149.0)
#   return image.addBands(laiBand, None, True).addBands(thermalBands, None, True)
...
def main(argv):
    ...
    # Get Sun position and overpasstime
    landsat_coll = landsat_coll.sort('system:time_start')

    SAA = landsat_coll.aggregate_array('SUN_AZIMUTH').getInfo()
    SEA = landsat_coll.aggregate_array('SUN_ELEVATION').getInfo()
    SZA = 90 - np.array(SEA)
    # VZA = landsat_coll.aggregate_array('ROLL_ANGLE').getInfo()
    overpass_time_utc = landsat_coll.aggregate_array('system:time_start').getInfo()
    ...
    metadata = pd.DataFrame({'OST': overpass_solar_time, 'SAA': SAA, 'SZA': SZA})
    metadata.to_csv(vector_dic[farm]['out_dir'] + '/METADATA.CSV')
    ...
    for i in range(n):
        ...
        thermal_band_name = {'LANDSAT_5': 'ST_B6', 'LANDSAT_7': 'ST_B6', 'LANDSAT_8': 'ST_B10', 'LANDSAT_9': 'ST_B10'}
        ...
        filename = laiImage.get("system:index").getInfo()

        out_dir = vector_dic[farm]['out_dir']
        output = out_dir + '/' + filename + '.tif'
        print(output)
        geemap.ee_export_image(
            laiScaled.select(['LAI', 'TRAD']),
            filename=output,
            scale=30,
...
if __name__ == "__main__":
    ...
    folder_id = vector_dic[farm]['folder_id']
    PATH = vector_dic[farm]['PATH']
    ROW = vector_dic[farm]['ROW']

    test_args = [
        "ee_Landsat_LAI_export_v0.2.0.py",
        "-o", folder_id,
        "-p", "43",
        "-r", "34",
        "-s", first_date,
